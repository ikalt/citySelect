...
describe("@ikalt/city-select-core", () => {
  ...
  it("creates structured province-city-district selection results", () => {
    ...
    expect(结果.完整路径.map((城市) => 城市.名称)).toEqual([
      "浙江省",
      "杭州市",
      "西湖区",
    ])
  ...
  it("creates structured four-level and variable-depth region paths", () => {
    const 浙江: 城市 = {
      编码: "330000",
      名称: "浙江省",
      拼音: "zhejiang",
    ...
    const 四级结果 = 创建行政区选择结果([浙江, 杭州, 西湖, 北山])
    const 台湾结果 = 创建行政区选择结果([台湾, 台北])

    expect(四级结果.名称路径).toEqual(["浙江省", "杭州市", "西湖区", "北山街道"])
    expect(四级结果.乡镇街道?.名称).toBe("北山街道")
    expect(台湾结果.名称路径).toEqual(["台湾省", "台北市"])
    expect(台湾结果.区县).toBeUndefined()
  })
