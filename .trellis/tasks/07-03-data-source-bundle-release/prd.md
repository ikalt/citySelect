...
- [ ] 当前分支远端状态和 PR 创建状态被记录；如果自动 PR 失败，给出明确原因和手动 URL。
- [ ] 生成脚本支持 source manifest，能表达层级级别的数据来源和 cutoff。
- [ ] 数据产物拆分为轻量入口和按需加载入口，默认导入不再解析完整行政区数据。
- [ ] 提供至少一个按省懒加载行政区路径示例，且 Taro/demo 能展示懒加载路径。
- [ ] 同步兼容 API 的保留或迁移策略写入文档，并有测试覆盖。
- [ ] 发布流程文档和 package metadata 规划完成；如果本任务实施发布准备，发布前 dry-run 或等价验证通过。
- [ ] 包体积检查命令或报告产物可复现。
- [ ] 完成数据源研究记录，包含官方源、新近第三方源、现用源的覆盖层级、日期、授权和稳定性比较；若研究结果不足以替换当前源，则记录 defer 原因和下一步。
